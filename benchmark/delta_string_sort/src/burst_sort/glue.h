...
void sinha_C_burstsort_prepare(string *strings, size_t size) {
  ...
  LOCHAR = 1;
  while (!CHARCOUNT[LOCHAR])
    ++LOCHAR;
  HICHAR = 255;
  while (!CHARCOUNT[HICHAR])
    --HICHAR;
}
...
void sinha_C_burstsort(string *strings, size_t size) {
  ...
  while (BINSIZE0 < MAXKEYLEN)
    BINSIZE0 *= 2; /* must fix at least one key */
