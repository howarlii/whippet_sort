...
#define DUMP                                                                   \
  {                                                                            \
    ...
    cr();                                                                      \
    isaye(NCHARS, "NCHARS");                                                   \
    isaye(MAXKEYLEN, "MAXKEYLEN");                                             \
    cr();                                                                      \
    isaye(NSEGS, "NSEGS");                                                     \
    isaye(SEGSIZE, "SEGSIZE");                                                 \
    cr();                                                                      \
    isaye(BINSIZE0, "BINSIZE0");                                               \
    isaye(LIMSIZE0, "LIMSIZE0");                                               \
    cr();                                                                      \
    isaye(FREEBURSTS, "FREEBURSTS");                                           \
    isaye(SAMPLERATE, "SAMPLERATE");                                           \
    cr();                                                                      \
    isaye(INPUTORDER, "INPUTORDER");                                           \
    isaye(WRITEFILE, "WRITEFILE");                                             \
    cr();                                                                      \
    isaye(CACHESIZE, "CACHESIZE");                                             \
    isaye(NODES, "NODES");                                                     \
    cr();                                                                      \
    isaye(BINS, "BINS");                                                       \
    isaye(NULLBINS, "NULLBINS");                                               \
    cr();                                                                      \
    isaye(ALLOCATED, "ALLOCATED");                                             \
    isaye(MAXALLOCATED, "MAXALLOCATED");                                       \
    br();                                                                      \
  }
...
/* globals */
size_t MAXKEYS,     /* maximum keys to read and sort (arg) */
...
double TMIN,       /* lowest time required by a repeat of a sort */
    TNORM;         /* equals TMIN/(NKEYS * log10(NKEYS)) */
char *INPUTDIR,    /* directory containing input files */
...
string INBUF,    /* input buffer used in CP- and CPL-burstsort */
    *SEGMENTS,   /* segmented input buffer used in C-burstsort */
    *SEGLIMITS;  /* limits for the segments of SEGMENTS	 */
FILE *ERRORLOG;  /* error log */
test TEST;       /* set by setsort() to be run in doreps() */
timer *TMR,      /* timer for current timing phase */
