...
void sbs(string *seg, string *seglim) {
  ...
  FREE(seg, NSEGS * sizeof(string));
  FREE(seglim, NSEGS * sizeof(string));
  ...
  if (WRITEFILE) {
    ...
    ton(tw);
    ssavetrie(rt, OUTPUTDIR, DATANAME);
  }
...
void ssample(string *seg, string *seglim, NODE1 *rt, int n) {
  ...
  while (n--) {
    ...
    /* If c is a NULL, the key is exhausted; when this happens in ssample(),
            we simply ignore that key, since it does not affect bursting and
       thus does not influence the trie structure formed during sampling */
    if (c == 0)
      continue;
    ...
    if (ct == 1) {
      ...
      /* Set the bin's limit pointer to its base + LIMSIZE0, where
              LIMSIZE0 = BINSIZE0 - MAXKEYLEN; MAXKEYLEN is guaranteed to be >=
         to the longest possible key length. */
      bn->lp = s + LIMSIZE0;
  ...
  for (c = LOCHAR; c <= HICHAR; ++c) {
    ...
    /* For other non-empty bins... */
      ...
      /* If the bin did not actually burst during sampling,
              ...
              preemptively. */
      if (((ct * sizeof(string *)) + bn->ap - bn->bp) * SAMPLERATE >
          CACHESIZE) {
        samburst(bn);
        clear((NODE1 *)bn->bp);
      }
...
void sadd(string b, string blim, NODE1 *rt) {
  ...
  while (b < blim) {
    ...
    /* We ignored exhausted keys during sampling (since they do not lead to
            ...
            key.	Such keys have no tail left to be copied to a bin, and
       are recorded only as counts, but they can be reconstructed from the chars
            traversed to reach a given bin.	I.e., the exhausted keys counted
       in a bin are identical to the prefix traversed to reach it. */
    if (c == 0) {
      if (ct == 1)
        ++NULLBINS;
      continue;
...
void sburst(NODE1 *bg) {
  ...
  do {
    ...
    while (n--) {
      ...
      if ((p = bg->bp) == NULL) {
        ++BINS;
        s = bg->bp = (string)MALLOC(BINSIZE0);
        bg->lp = s + LIMSIZE0;
        while ((*s++ = *b++) != 0)
          ;
        bg->ap = s;
...
void straverse(NODE1 *nd) {
  ...
  for (c = LOCHAR; c <= HICHAR; ++c) {
    ...
    /* If the bin contains more than one string tail, sort it; multikey
            ...
            high branching; faster sorting can be achieved by adaptively
       choosing mkq when the ratio of terminal bins to branched nodes < 25 and
       radix when it is greater. */
    if (ct > 1)
      mkqsort(tp0, ct, 0);
  }
}
