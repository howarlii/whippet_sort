...
def run_benchmark(data_path, method):
    time_to_sleep = 0.5
    std_dev_lmt = 0.15
    while True:
...
def run_benchmark_and_draw(data_name, seeds=["0"]):
    step_time_avgs = dict()
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        ...
        for method in method_args:
            ...
            for seed in seeds:
                ...
                tasks[method][seed] = executor.submit(
                    run_benchmark, data_path, method)
                time.sleep(0.5)


        for method0 in method_args:
            for seed in seeds:
                result = tasks[method0][seed].result()
                if result is None:
                    continue
                for method, steps in result.items():
                    if method not in step_time_avgs:
                        step_time_avgs[method] = dict()
                    for step, time_ms in steps.items():
                        if step not in step_time_avgs[method]:
                            step_time_avgs[method][step] = []
                        step_time_avgs[method][step].append(time_ms)
        # Average the values in the same slot
        for method in step_time_avgs:
            for step in step_time_avgs[method]:
                t = step_time_avgs[method][step]
                step_time_avgs[method][step] = sum(t) / len(t)
    ...
    for j, (method, steps) in enumerate(step_time_avg.items()):
        ...
        for k, (step_name, step_time) in enumerate(steps.items()):
            if step_time == 0:
                continue
            # 绘制每个步骤的柱子部分
            axs.bar(index[j], step_time, bar_width, bottom=bottom,
                        label=step_name if j == 0 else "", color=colors[k % len(colors)])

            # 在图中添加步骤名称和对应的时间
            axs.text(index[j], bottom + step_time / 2,
        ...
        # 在每个柱子的顶端添加总耗时
        axs.text(index[j], bottom + 10, f'Total\n{int(total_time)}ms',
                    ha='center', va='bottom', color='black', fontsize=10)

    # 添加标签和标题
    axs.set_xlabel('Methods')
    axs.set_ylabel('Time (ms)')
    axs.set_title(f'{title_str}')
    axs.set_xticks(index)
    axs.set_xticklabels(methods)
    ...
    # 减少柱状图之间的间隙
    axs.set_xticklabels(methods, rotation=45, ha="right")
...
def main():
    ...
    for row_num in row_sizes:
        results[row_num] = run_benchmark_and_draw(f"{row_num}", ["0", "19260817", "114514", "1919810"])
